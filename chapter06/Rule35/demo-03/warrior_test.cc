...
class WarriorTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    w1.set_do_under_attack(&WarriorUnderAttack);
    w1.set_age(20);

    w2.set_hv(100);
    w2.set_level(kIntermediate);
    w2.set_do_under_attack(&WarriorUnderAttack);
    w2.set_age(21);

    w3.set_hv(100);
    w3.set_level(kSenior);
    w3.set_do_under_attack(&WarriorUnderAttack);
    w3.set_age(22);
  }
...
TEST_F(WarriorTest, DoUnderAttackWorks) {
  ...
  EXPECT_EQ(w1.do_under_attack(), &WarriorUnderAttack);
  EXPECT_EQ(w2.do_under_attack(), &WarriorUnderAttack);
  EXPECT_EQ(w3.do_under_attack(), &WarriorUnderAttack);
}
...
TEST_F(WarriorTest, SetDoUnderAttack) {
  w0.set_do_under_attack(&WarriorUnderAttack);
  EXPECT_EQ(w0.do_under_attack(), &WarriorUnderAttack);
}
